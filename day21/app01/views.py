...
def login(request):
    ...
    if request.method == "POST":
        ...
        if dic['pwd'] == p:
            ...
            # import datetime
            # current_date = dme.atetime.datetiutcnow()
...
def cookie(request):
    ...
    current_date = current_date + datetime.timedelta(seconds=5)#5秒过期
    response.set_cookie('username111',"value",expires=current_date)#path="/"指定使用cookie的路径
    response.set_cookie('username111',"value",max_age=10)
